...
## Priors
  ...
  **Equation**:  $b_r \sim {\sf Normal}(0, 0.01)$ where $r \in 1:n_{par}$   
  ...
  **Equation:** $b_{n.par+1:n.spl+n.par} \sim {\sf Normal}(Z_{n.par+1:n.spl+n.par}, \sigma_{pre})$  
  ...
  **Equation:** $b_{n.X-n.spl+1:n.X} \sim {\sf Normal}(Z_{n.X-n.spl+1:n.X}, \sigma_{pos})$  
      
 - Priors for smoothing parameter  
 
  **JAGS code:** `for (f in 1:n.fac) { gamma[f] ~ dgamma(.5,.5)}`   
  **Equation:** $\gamma_{f} \sim {\sf Gamma}(0.5, 0.5)$   
...
## Likelihood
  ...
  **Equation:** $P_{ret_j} = \alpha_{0_{country_j}} \times \exp^{-\alpha_1 \times acce_j}$  
...
## Predictions
  ...
  **Equation:** $A_{pos} = \sum \eta_{pred_j}$ where $j \in n_{PO}/2:n_{PO}$  
